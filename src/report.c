...
int cw_html_report_begin_mode(FILE *stream, const char *mode)
{
    static const char header[] =
        ...
        "stroke-linecap:round;stroke-linejoin:round}.seq-submit{fill:none;"
                       ...
                       size_t count)
{
    static const char footer[] =
        ...
        "const parts=h=>{if(h.work_kind)return[{n:'queue',v:h.queue_ns,c:'queue'},"
        ...
        "='hop '+h.index+' · '+h.source+' → '+h.target+(h.target_arg?' arg'+h.target_arg:'');"
        "const span=document."
        ...
        "return source?source.source:threadName(c,tid)}"
        ...
        "width=1180,height=500,left=105,top=100,bottom=52,plotHeight=height-top-bottom,"
        "sourceX=250,loopX=590,actionX=930,phases=[initialDelay,dispatchDelay,execution],"
        ...
        "title.textContent=runtime+' event lifecycle';desc.textContent='Waiting for readiness, ready-to-dispatch delay, and callback or I/O execution are shown as separate phases. Role lanes may share one thread.';"
        "svg.append(title,desc);const defs=svgNode('defs');"
        "function arrow(id,color){const marker=svgNode('marker',{id,viewBox:'0 0 10 10',refX:'8',"
        "refY:'5',markerWidth:'6',markerHeight:'6',orient:'auto-start-reverse'});"
        "marker.appendChild(svgNode('path',{d:'M 0 0 L 10 5 L 0 10 z',fill:color}));"
        ...
        "if(waitPhase)lane(sourceX,'epoll readiness',precursor.source);"
        "else if(precursor)lane(sourceX,precursor.source,'wake source · TID '+precursor.tid);"
        "else lane(sourceX,'readiness observed','capture begins at ready');"
        "const loopTid=operation?operation.tid:(precursor?precursor.target_tid:first.tid);"
        "lane(loopX,runtime+' event loop','TID '+loopTid);"
        "if(operation)lane(actionX,operation.target,'same TID '+operation.target_tid,360);"
        "svg.appendChild(svgNode('circle',{cx:sourceX,cy:top,r:4,class:'seq-dot'}));"
        "if(waitPhase){if(initialDelay){const wait=svgNode('rect',{x:sourceX-8,y:top,width:16,"
        "height:Math.max(3,readyY-top),rx:3,class:'seq-wait'});"
        "svgTitle(wait,'waiting for readiness: '+exact(initialDelay));svg.appendChild(wait);"
        "svgText(svg,sourceX+20,(top+readyY)/2+4,'waiting for readiness · '+exact(initialDelay),"
        "'seq-label','start')}const readyPath=svgNode('path',{class:'seq-submit',"
        "'marker-end':'url(#event-neutral-arrow)',d:'M '+sourceX+' '+readyY+' H '+loopX});"
        "svgTitle(readyPath,precursor.target);svg.appendChild(readyPath);"
        "svgText(svg,(sourceX+loopX)/2,readyY-9,precursor.target==='timeout'?'wait timeout':'FD ready',"
        "'seq-sub','middle')}else if(precursor){const wakePath=svgNode('path',{class:'seq-queue',"
        "'marker-end':'url(#event-delay-arrow)',d:'M '+sourceX+' '+top+' H '+(sourceX+54)+"
        "' V '+readyY+' H '+loopX});svgTitle(wakePath,'wake-to-ready: '+exact(initialDelay));"
        "svg.appendChild(wakePath);if(initialDelay)svgText(svg,sourceX+68,(top+readyY)/2+4,"
        "'wake-to-ready · '+exact(initialDelay),'seq-queue-label','start')}"
        "else{const readyPath=svgNode('path',{class:'seq-submit','marker-end':'url(#event-neutral-arrow)',"
        "d:'M '+sourceX+' '+top+' H '+loopX});svg.appendChild(readyPath);"
        "svgText(svg,(sourceX+loopX)/2,top-9,'ready observed','seq-sub','middle')}"
        "svg.appendChild(svgNode('circle',{cx:loopX,cy:readyY,r:5,class:'seq-dot'}));"
        "if(operation){const delayName=operation.work_kind===2?'ready-to-I/O':'ready-to-callback',"
        "delayPath=svgNode('path',{class:dispatchDelay?'seq-queue':'seq-submit',"
        "'marker-end':dispatchDelay?'url(#event-delay-arrow)':'url(#event-neutral-arrow)'}),"
        "elbowX=loopX+70,corner=Math.min(8,Math.max(0,(actionY-readyY)/2));"
        "if(dispatchDelay&&corner>=2)delayPath.setAttribute('d','M '+loopX+' '+readyY+' H '+"
        "(elbowX-corner)+' Q '+elbowX+' '+readyY+' '+elbowX+' '+(readyY+corner)+' V '+"
        "(actionY-corner)+' Q '+elbowX+' '+actionY+' '+(elbowX+corner)+' '+actionY+' H '+actionX);"
        "else delayPath.setAttribute('d','M '+loopX+' '+readyY+' H '+actionX);"
        "svgTitle(delayPath,delayName+': '+exact(dispatchDelay));svg.appendChild(delayPath);"
        "if(dispatchDelay){svg.appendChild(svgNode('rect',{x:elbowX-5,y:readyY,width:10,"
        "height:Math.max(3,actionY-readyY),rx:5,class:'seq-queue-band'}));"
        "svgText(svg,elbowX+14,(readyY+actionY)/2+4,delayName+' · '+exact(dispatchDelay),"
        "'seq-queue-label','start')}else svgText(svg,(loopX+actionX)/2,readyY-9,"
        "delayName+' · 0 ns','seq-sub','middle');"
        "svg.appendChild(svgNode('circle',{cx:actionX,cy:actionY,r:5,class:'seq-dot'}));"
        "if(execution){const work=svgNode('rect',{x:actionX-8,y:actionY,width:16,"
        "height:Math.max(3,finalY-actionY),rx:3,class:'seq-work'}),"
        "executionName=operation.work_kind===2?'I/O execution':'callback execution';"
        "svgTitle(work,executionName+': '+exact(execution));svg.appendChild(work);"
        "svgText(svg,actionX-18,(actionY+finalY)/2+4,executionName+' · '+exact(execution),"
        "'seq-label','end')}}"
        "const finalX=operation?actionX:loopX;svg.appendChild(svgNode('circle',"
        "{cx:finalX,cy:finalY,r:6,class:'seq-dot'}));svgText(svg,finalX,"
        "Math.min(height-12,finalY+23),'event complete · '+exact(total),'seq-label','middle');"
        "root.appendChild(svg)}"
        ...
        "const phases=[];c.hops.forEach(h=>{phases.push(h.queue_ns,h.work_ns)});"
        ...
        "svgTitle(path,'hop '+h.index+' queue: '+exact(h.queue_ns)+' · '+h.source+' → '+h.target);"
        "if(h.queue_ns)group.appendChild(svgNode('rect',{x:elbowX-5,y:sourceY,width:10,"
        ...
        "'H'+h.index+' · queued '+exact(h.queue_ns),'seq-queue-label',queueAnchor);"
        ...
        "$('sequenceMeta').textContent=chainDescription(c,i);renderSequence(c,i);"
        ...
        "$('reportSubtitle').textContent='Captured readiness, event-loop dispatch, and callback or I/O execution phases.';"
        "$('selectedQueueLabel').textContent='Selected wait/delay';"
        "$('selectedWorkLabel').textContent='Selected execution';"
        "$('sequenceTitle').textContent=runtimeName+' event lifecycle';"
        "$('sequenceNote').textContent='Role lanes separate epoll readiness, the event loop, and callback or I/O execution even when they share one thread. Gray is waiting for readiness, violet is wake or dispatch delay, and blue is callback or I/O execution.'}"
